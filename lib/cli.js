...
async function run(argv, dependencies = {}) {
  ...
  const result = await generateCurrentTotp(input, options, dependencies);
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    switch (arg) {
      ...
      case "-q":
        ...
        break;
      case "--no-wait":
        options.waitForFreshCode = false;
        break;
      case "--help":
      case "-h":
        options.help = true;
...
async function generateCurrentTotp(input, options, dependencies = {}) {
  ...
  let result = generateTotp({
    ...request,
    timestamp: options.timestamp ?? now()
  });

  if (!shouldWaitForFreshCode(result, options)) {
    return result;
  }

  await sleep(FRESH_CODE_WAIT_MS);
  result = generateTotp({
    ...request,
    timestamp: now()
  });
...
function shouldWaitForFreshCode(result, options) {
  return options.waitForFreshCode && options.timestamp === undefined && result.remainingSeconds < MIN_FRESH_SECONDS;
}
...
By default, if the current code has less than ${MIN_FRESH_SECONDS} seconds left,
the CLI waits ${FRESH_CODE_WAIT_MS / 1000} seconds and returns a fresh code.
